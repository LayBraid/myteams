...
static int response(client_t *client)
{
    ...
    strcpy(response->thread_uuid, client->context_thread);
...
int use_thread(server_t *server, client_t *client,
s_use_request_t *request, int restore)
{
    ...
    if (!find_thread(channel, request->thread_uuid)) return 0;
    thread = get_thread(channel, request->thread_uuid);
    strcpy(client->context_team, team->uuid);
    strcpy(client->context_channel, channel->uuid);
    strcpy(client->context_thread, thread->uuid);
